...
import pygame, Numeric, os
from pygame.locals import *
from math import *

pickerWheel = "pytari.wheel.tga"

circleSize = 127    # Resulting image is Size * 2 + 1
circleAlias = 4     # Antialiasing grid size, Larger grid is more accurate
...
if not pickerWheel in os.listdir( os.getcwd() ):
    ...
    for xx in dimRange:
        for yy in dimRange:
            color = Numeric.zeros((3)).astype("l")
            alphaAliased = 0
                ...
                for ys in aliasRange:
                    ...
                    if saturation <= 1.0:
                        if y != 0:
                            ...
                            hue = 90
                        color += Numeric.array(ColorHSV( hue, saturation, 255 )).astype("b")
                        alphaAliased += 255
